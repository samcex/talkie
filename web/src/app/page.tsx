...
export default function HomePage() {
  ...
  async function startDirectCall(target: DirectUser | Contact) {
    ...
    const callBody = callRes?.ok
      ? ((await callRes.json().catch(() => null)) as {
          call?: { id?: string; expiresAt?: number };
        } | null)
      : null;
    const callQuery =
      callBody?.call?.id && callBody.call.expiresAt
        ? `&call=${encodeURIComponent(callBody.call.id)}&expires=${encodeURIComponent(
            String(callBody.call.expiresAt),
          )}`
        : '';
    router.push(
